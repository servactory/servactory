...
RSpec.describe Usual::TestKit::Rspec::AllowServiceAsFailure::Example2, type: :service do
  describe ".call!" do
    subject(:perform) { described_class.call! }

    include_examples "check class info",
                     inputs: %i[],
                     internals: %i[child_result],
                     outputs: %i[]

    context "when the input arguments are valid" do
      describe "but the data required for work is invalid" do
        before do
          allow_service_as_failure(Usual::TestKit::Rspec::AllowServiceAsFailure::Example2Child) do
