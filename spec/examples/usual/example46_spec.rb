        ...
        describe "because one element has the wrong type" do
          ...
          it "returns expected error" do
            expect { perform }.to(
              raise_error(
                ApplicationService::Errors::InputError,
                "[Usual::Example46] Wrong type in input collection `invoice_numbers`, expected `String`, got `Integer`"
              )
            )
          end
        end
        ...
        describe "because one element is empty" do
          ...
          it "returns expected error" do
            expect { perform }.to(
              raise_error(
                ApplicationService::Errors::InputError,
                "[Usual::Example46] Required element in input collection `invoice_numbers` is missing"
              )
            )
          end
        end
