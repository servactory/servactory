...
RSpec.describe Usual::Example66 do
  describe ".call!" do
        ...
        describe "because one element is empty" do
          ...
          it "returns expected error" do
            expect { perform }.to(
              raise_error(
                ApplicationService::Errors::InputError,
                "[Usual::Example66] Required element in input collection `ids` is missing"
              )
            )
          end
        end
    ...
    context "when the input arguments are invalid" do
      context "when `ids`" do
        ...
        it_behaves_like "input type check",
                        name: :ids,
                        array: true,
                        array_message: "Input `ids` must be a collection of `String`",
