...
RSpec.describe Usual::Predicate::Example1, type: :service do
  describe ".call!" do
    ...
    context "when the input arguments are invalid" do
      it { expect { perform }.to have_input(:enable).simulation(attributes).types(TrueClass, FalseClass).required }
      it { expect { perform }.to have_input(:text).simulation(attributes).type(String).required }
      it { expect { perform }.to have_input(:number).simulation(attributes).type(Integer).required }
    end
  end
