...
RSpec.describe Usual::Predicate::Example1, type: :service do
  describe ".call!" do
    ...
    context "when the input arguments are invalid" do
      it { expect { perform }.to have_input(:enable).direct(attributes).types(TrueClass, FalseClass).required }
      it { expect { perform }.to have_input(:text).direct(attributes).type(String).required }
