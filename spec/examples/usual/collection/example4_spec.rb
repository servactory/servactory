...
RSpec.describe Usual::Collection::Example4, type: :service do
  describe ".call!" do
          ...
          it "returns expected error" do
            expect { perform }.to(
              raise_error(
                ...
                "[Usual::Collection::Example4] Wrong type in input collection `ids`, expected `String`, got `Integer`"
              )
            )
          end
        end
        ...
        describe "because one element is empty" do
          ...
          it "returns expected error" do
            expect { perform }.to(
              raise_error(
                ApplicationService::Exceptions::Input,
                "[Usual::Collection::Example4] Required element in input collection `ids` is missing"
              )
            )
          end
        end
    ...
    context "when the input arguments are invalid" do
      it { expect { perform }.to have_input(:ids).simulation(attributes).type(Array).consists_of(String).required }
    end
  end
