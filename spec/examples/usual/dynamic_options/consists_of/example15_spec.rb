...
RSpec.describe Usual::DynamicOptions::ConsistsOf::Example15, skip: "DELETE ME" do
  describe ".call!" do
        ...
        describe "because one element is empty" do
          ...
          it "returns expected error" do
            expect { perform }.to(
              raise_error(
                ApplicationService::Exceptions::Input,
                "[Usual::DynamicOptions::ConsistsOf::Example15] Required element in input collection `ids` is missing"
              )
            )
          end
        end
