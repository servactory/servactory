...
RSpec.describe Servactory::TestKit::Rspec::Matchers::Result::BeSuccessServiceMatcher do
  ...
  describe "#description" do
    it "returns 'service success'" do
      expect(matcher.description).to eq("service success")
    end
  end
  ...
  describe "#with_output" do
    ...
    it "returns self for chaining" do
      expect(matcher.with_output(:result, "TEST")).to eq(matcher)
    end
  end
  ...
  describe "#with_outputs" do
    ...
    it "returns self for chaining" do
      expect(matcher.with_outputs(result: "TEST")).to eq(matcher)
    end
  end
  ...
  describe "#failure_message" do
    ...
    context "with failure result" do
      before { matcher.matches?(failure_result) }

      it "describes success/failure mismatch", :aggregate_failures do
        expect(matcher.failure_message).to include("expected success")
        expect(matcher.failure_message).to include("got failure")
      end
    end

    context "with non-existent output key" do
      before do
        matcher.with_output(:nonexistent, "value")
        matcher.matches?(success_result)
      end

      it "describes the missing key", :aggregate_failures do
        expect(matcher.failure_message).to include("Non-existent value key")
        expect(matcher.failure_message).to include("nonexistent")
      end
    end

    context "with wrong output value" do
      before do
        matcher.with_output(:result, "wrong")
        matcher.matches?(success_result)
      end

      it "describes the value mismatch", :aggregate_failures do
        expect(matcher.failure_message).to include("Incorrect result value")
        expect(matcher.failure_message).to include("result")
        expect(matcher.failure_message).to include("wrong")
        expect(matcher.failure_message).to include("TEST")
      end
    end
  end
  ...
  describe "#failure_message_when_negated" do
    it "returns negation message" do
      expect(matcher.failure_message_when_negated).to include("not to be a successful service")
    end
  end
