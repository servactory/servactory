...
RSpec.describe Servactory::TestKit::Rspec::Matchers::Submatchers::Shared::SchemaSubmatcher do
  subject(:submatcher) { described_class.new(context, { key: { type: String } }) }
  ...
  describe "#description" do
    it "includes 'schema'" do
      expect(submatcher.description).to include("schema")
    end

    it "includes the schema structure" do
      expect(submatcher.description).to include("key")
    end
  end
  ...
  describe "#failure_message" do
    context "when match fails" do
      subject(:submatcher) { described_class.new(context, { wrong: { type: String } }) }

      before { submatcher.matches?(nil) }

      it "includes expected schema" do
        expect(submatcher.failure_message).to include("wrong")
      end

      it "includes actual schema" do
        expect(submatcher.failure_message).to include("key")
      end

      it "describes the failure" do
        expect(submatcher.failure_message).to include("schema")
      end
    end
  end
end
