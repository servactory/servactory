    ...
    module Workspace
      class Internals
        ...
        def fetch_with(name:, &block) # rubocop:disable Metrics/MethodLength, Metrics/AbcSize, Lint/UnusedMethodArgument
          internal_name = @context.class.config.config.predicate_methods_enabled ? name.to_s.chomp("?").to_sym : name
          ...
          if name.to_s.end_with?("?") && @context.class.config.config.predicate_methods_enabled
